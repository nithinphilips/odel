...
If the user did not specify the ``--wait`` option, Odel will then quits.

If the ``--wait`` option is on, Odel needs to find out if the records that were
just uploaded were processed. Tririga processes Data Integrator uploads
...
file with the same name was uploaded previosly) to see if any of them are in
one of the processing statuses (``NEW``, ``DONE`` or ``UPLOADING...``.) If they
are, Odel waits a few seconds a checks again. It repeats the process until the
status changes. When the status changes Odel quits. If the file is not
